...
const imageSize = async (image) => {
  ...
  try {
    ...
    warn(`Cannot fetch image size for ${image} - manifest fetch error.`);
    return 0;
  } catch (e) {
    warn(e.message);
    return 0;
  ...
  for (const pipeline of pipelines) {
    for (const job of pipeline.jobs) {
      ...
      if (data) {
        ...
        job.imageSize = sizes[data.image];
      }
    }
    pipeline.imageSizes = pipeline.jobs.reduce(
      (previous, job) => previous + job.imageSize,
      0,
    );
  }
  return pipelines;
...
const percentiles = (data, property) => {
  ...
  return [
    list[Math.floor(list.length / 2)][property],
    list[Math.floor((3 * list.length) / 4)][property],
  ];
};
...
const formatPercentiles = (description, unit, indent, list) => {
  return [
    `${''.padEnd(indent)}${c.greenBright(description)}`,
    `${''.padEnd(indent + 2)}${c.white('Median         : ')} ${c.yellowBright(formatUnit(list[0], unit))}${unit}`,
    `${''.padEnd(indent + 2)}${c.white('75th Percentile: ')} ${c.yellowBright(formatUnit(list[1], unit))}${unit}`,
  ].join('\n');
};
...
export const analyzer = {
  ...
  rule: (key, obj, depth, resolver, node) => {
    ...
    if (depth === 0 && key === 'default' && Object.hasOwn(obj, 'image')) {
    ...
    if (depth === 0 && !allKeywords.has(key)) {
      ...
      if (Object.hasOwn(obj, 'image')) {
        ...
        const full = resolver();
        if (full && Object.hasOwn(full, 'image')) {
    ...
    const description = [
      ...
      `${formatPercentiles('Duration:', 's', 4, percentiles(pipelines, 'duration'))}`,
      `${formatPercentiles('Accumulated images size:', 'KB', 4, percentiles(pipelines, 'imageSizes'))}`,
    ...
    for (const job in jobs) {
      ...
      description.push(
        `${formatPercentiles('Duration:', 's', 6, percentiles(jobs[job], 'duration'))}`,
      );
      description.push(
        `${formatPercentiles('Image size:', 'KB', 6, percentiles(jobs[job], 'imageSize'))}`,
      );
    }
