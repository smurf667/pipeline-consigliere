...
const formatPercentiles = (description, unit, indent, list, timeout) => {
  return [
    `${''.padEnd(indent)}${c.greenBright(description)}`,
    `${''.padEnd(indent + 2)}${c.white('Median         : ')} ${c.yellowBright(formatUnit(list[0], unit))}${unit}`,
    `${''.padEnd(indent + 2)}${c.white('75th Percentile: ')} ${c.yellowBright(formatUnit(list[1], unit))}${unit} ${timeout ? 'job timeout: ' + c.yellowBright(timeout) : ''}`,
  ].join('\n');
};
...
export const analyzer = {
  ...
  rule: (key, obj, depth, resolver, node) => {
    ...
    for (const job in jobs) {
      ...
      description.push(
        `${formatPercentiles('Duration:', 's', 6, percentiles(jobs[job], 'duration'), analyzer.timeouts[job])}`,
      );
      description.push(
